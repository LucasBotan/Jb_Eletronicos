...
namespace CRMagazine
{
    ...
    {
        public frmReparoPPRecebido(string texto)
        {
            InitializeComponent();
      ...
      //  public string OS = "";
        ...
        {
            string sql = "";
            sql += " Select * from estoque where chamado = '" + Chamado + "' ";
            cx.ConectarSAP();
            SqlDataAdapter da = new SqlDataAdapter(sql, cx.c);
            DataSet ds = new DataSet();
            da.Fill(ds, "Chamados");
        ...
        {
            string sql = "";
            sql += "SELECT Status, '' AS DISPONÍVEL, CHAMADO, '' AS [PEÇA SOLICITADA], Codigo AS [PEÇA ENTREGE], DESCRICAO FROM AGUARDOBACKUP WHERE Chamado = '" + Chamado + "'";  
        ...
        public void ChecarSePPRecebido2()
        {
            for (int i = 0; i < dgvPP.RowCount; i++)
            {
                ...
                try
                {
                    ...
                    string comando = "";
                    comando = "select * from Estoque where Chamado = '" + Cham + "' AND Codigo = '" + pecaEntregue + "' and Posicao = 'BACKUP'";
                    cx.Conectar();
        ...
        public void ChecarSePPRecebido()
        {
            for (int i = 0; i < dgvPP.RowCount; i++)
            {
                ...
                try
                {
                    ...
                    string comando = "";
                    comando = "select * from PP where Chamado = '" + Cham + "'";
                    cx.Conectar();
                    SqlCommand cd = new SqlCommand();
                    cd.Connection = cx.c;
                    cd.CommandText = comando;
        ...
        */
        ...
        public void SolicitarPeca(string Cod, string Des, string Chamado)
        {
            ...
            consulta.comando = "";
            consulta.comando = "Insert into Pedidos (Usuario, Codigo, Descricao, Quantidade, Status, HoraPedido, Situacao, NS, OS) Values ";
            consulta.comando += " ('" + lblUsuario.Text + "', ";
        ...
        {
            consulta.comando = "";
            consulta.comando = "Delete from Estoque where Codigo = '" + Codigo + "' and Chamado = '" + Chamado + "' ";
            consulta.Atualizar();
            //MessageBox.Show(consulta.comando);
        }
            ...
            if (dgvPP.RowCount > 0)
            {
                ...
                if (feedBack == "SIM")
                {
                    ...
                    if (consulta.Retorno == "ok")
                    {
                        ...
                        consulta.DataAtual();
                        consulta.InsereHistorico(Chamado, lblUsuario.Text, StatusHistorico, consulta.dataNormal, consulta.hora);
                        //=====fim da inserção======================================
                        consulta.comando = "";
                        consulta.comando = "update Chamados set Status = 'REPARO' where OS = '" + Chamado + "'";
                        consulta.Atualizar();
                        // MessageBox.Show(consulta.comando);
