...
namespace CRMagazine
{
    ...
    {
        public frmEstoqueConferePecaBoa()
        {
            InitializeComponent();
        }
        ...
        {
            string sql = "";
            sql += " Select idPedidos, Usuario, NS, OS, Status, Codigo, Descricao, Quantidade, HoraFinalizacao from Pedidos where Status in ('RETORNO', 'RMDF')";
            cx.Conectar();
        ...
        {
            string sql = "";
            sql += " select idCod, Codigo, Barebone, Descricao, Quantidade, Posicao from Estoque where Codigo = '" + codigo + "' and Posicao != 'PP'";
            cx.Conectar();
            SqlDataAdapter da = new SqlDataAdapter(sql, cx.c);
            DataSet ds = new DataSet();
            da.Fill(ds, "Estoque");
        ...
        public void AtualizaPedidos()
        {
            consulta.DataAtual();
            consulta.comando = "";
            consulta.comando = "update Pedidos set Status = 'RETORNO-OK', HoraMovimentacao = '" + consulta.dataCompleta + "' where idPedidos = '" + idPedidos + "' ";
            consulta.Atualizar();
        ...
        public void AtualizaPedidosRMDF()
        {
            consulta.DataAtual();
            consulta.comando = "";
            consulta.comando = "update Pedidos set Status = 'RMDF-OK', HoraMovimentacao = '" + consulta.dataCompleta + "' where idPedidos = '" + idPedidos + "' ";
            consulta.Atualizar();
            // MessageBox.Show(consulta.comando);
        }
        ...
        public void AtualizaDestino()
        {
            ...
            consulta.comando = "";
            consulta.comando = "update Estoque set Quantidade = '" + total + "' where Posicao = '" + txtRetirar.Text + "' and Codigo = '" + codigo + "'";
           // MessageBox.Show(consulta.comando);
            consulta.Atualizar();
        ...
        public void AtualizaLocal()
        {
            ...
            consulta.comando = "";
            consulta.comando = "update Estoque set Quantidade = '" + total + "' where Posicao = '" + Posicao + "' and Codigo = '" + codigo + "'";
            consulta.Atualizar();
            Reset();
        }
         */ 
            ...
            if (e.KeyChar == 13)
            {
                if (Status == "RMDF")
                {
                    if (MessageBox.Show("DESEJA CONCLUIR RMDF? ", "Pergunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        consulta.comando += "('" + Usuario + "', '" + OS + "', '" + Chamado + "', '" + codigo + "', '" + Descricao + "', '1', 'CONFIRMADO-RMDF', '" + consulta.data + "', 'SEM ASSIST', '" + consulta.data + "') ";
                        consulta.Atualizar();
                        //  MessageBox.Show(consulta.comando);                                             
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            consulta.comando = "";
                            consulta.comando += "Select * from Estoque where Codigo = '" + txtCodigo.Text + "' and Posicao = '" + txtRetirar.Text + "'";
                            //MessageBox.Show(consulta.comando);
