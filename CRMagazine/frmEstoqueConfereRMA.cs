...
namespace CRMagazine
{
    public partial class frmEstoqueConfereRMA : Form
    {
        ...
        public void ListarTudo()
        {
            ...
            sql += " Select idRMA, Usuario, Chamado, OS, CodAntigo, DescPecaAntiga, Codigo, Descricao, Qntd, Status, Data, Situacao from RMA where Status like '%PENDENTE%'";      
            ...
            try
            {
                ...
                if (dgvConsulta.Columns[e.ColumnIndex].Name == "CONCLUIR")
                {
                    if (MessageBox.Show("DESEJA CONCLUIR? ", "Pergunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        consulta.DataAtual();
                        consulta.comando = "";
                        consulta.comando = "update RMA set Status = 'CONFIRMADO' where idRMA = '" + ID + "'";
                        consulta.Atualizar();
                ...
                else if (dgvConsulta.Columns[e.ColumnIndex].Name == "CANCELA")
                {
                    if (MessageBox.Show("DESEJA CANCELAR? ", "Pergunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        {
                            consulta.comando = "";
                            consulta.comando = "update Pedidos set Status = 'CANCELADO', HoraEntrega = '" + consulta.dataCompleta + "' where idPedidos = '" + ID + "'";
                            consulta.Atualizar();
                            if (consulta.Retorno == "ok")
                            {
