...
namespace CRMagazine
{
    ...
    {
        public frmExpedicaoPlanilhaNotaFiscal()
        {
            InitializeComponent();
        }
        ...
        public void ListarTudo()
        {
            ...
            sql += "case when Classificacao in ('GARANTIA', 'ORCAMENTO', 'NOVO') then 'NOVO' ELSE Classificacao END AS Classificacao,  ";
            sql += "count(case wheN Classificacao in ('GARANTIA', 'ORCAMENTO', 'NOVO') then 'NOVO' ELSE Classificacao END) as QNT, VAREJISTA ";
            sql += "from Chamados where Status = 'FINALIZADO' and NotaFiscal != '' and NotaFiscalSaida = 'PENDENTE' ";
            sql += "group by NotaFiscal, case wheN Classificacao in ('GARANTIA', 'ORCAMENTO', 'NOVO') then 'NOVO' ELSE Classificacao END, VAREJISTA ";
        ...
        public void ListarTudoPorFora()
        {
            ...
            sql += ", VAREJISTA ";
            sql += " from NOTAPORFORA where Status = 'PENDENTE' ";
        ...
        public void ConsultarItens(string NF, string Classi)
        {
           ...
           // sql += " where Status = 'FINALIZADO' and NotaFiscal = '" + NF + "' and Classificacao = '" + Classi + "' group by NotaFiscal, Classificacao, Descricao, CodVarejo ";
            ...
            sql += "case when Classificacao in ('GARANTIA', 'ORCAMENTO', 'NOVO') then 'NOVO' ELSE Classificacao END AS Classificacao, C.CodVarejo, ";
            //sql += "C.Descricao, ";
           ...
           // sql += " COUNT(C.CodVarejo) * CONVERT(FLOAT,N.Valor_uni) AS	VALOR_TOTAL ,N.NCM from Chamados C, NotaFiscal N ";
            sql += "replace(COUNT(C.CodVarejo) * CONVERT(NUMERIC(10,2),N.Valor_uni), '.', ',') AS VALOR_TOTAL ,N.NCM, N.EAN from Chamados C, NotaFiscal N ";
            sql += " where C.NotaFiscal = N.NotaFiscal and C.CodVarejo = N.CodVarejo and NotaFiscalSaida = 'PENDENTE' ";
            sql += " AND Status = 'FINALIZADO' and C.NotaFiscal = '" + NF + "' and C.Varejista = '" + txtVarejista.Text + "' and ";
            if (txtOrcamento.Text == "NOVO")
            {
                sql += "Classificacao in ('ORCAMENTO', 'GARANTIA', 'NOVO') ";
            }
            else
            {
                sql += "Classificacao = '" + Classi + "' ";
            }
            sql += "group by C.NotaFiscal, case when Classificacao in ('GARANTIA', 'ORCAMENTO', 'NOVO') then 'NOVO' ELSE Classificacao END, N.Descricao, C.Descricao, C.CodVarejo, N.Valor_uni, N.NCM, N.EAN ";
            cx.Conectar();
            SqlDataAdapter da2 = new SqlDataAdapter(sql, cx.c);
            DataSet ds2 = new DataSet();
            da2.Fill(ds2, "Chamados");
            ...
            try
            {                                
                sql = "select idChamados from Chamados where NotaFiscal = '" + NF + "' and NotaFiscalSaida = 'PENDENTE' AND ";
                if (txtOrcamento.Text == "NOVO")
                {
                    sql += "Classificacao in ('ORCAMENTO', 'GARANTIA', 'NOVO') ";
                }
        ...
        public void ConsultarItensPorfora(string NF, string Classi)
        {
            ...
            sql += " , replace(sum(convert(numeric,QNT)) * CONVERT(NUMERIC(10,2),Valor_uni), '.', ',') AS VALOR_TOTAL , NCM, '' as EAN from NotaPorFora ";
            ...
            try
            {
                sql = "select idNotaPorFora from NotaPorFora where NotaFiscal = '" + NF + "' and Status = 'PENDENTE' AND Classificacao = '" + Classi + "' ";
                cx.Conectar();
                SqlCommand cd = new SqlCommand();
                cd.Connection = cx.c;
                cd.CommandText = sql;
        ...
        {
            string sql = "";
            sql = "select OS, Descricao, NS, Status from Chamados where Notafiscal = '" + NF + "' and Status != 'EXPEDICAO' order by Status";
            cx.Conectar();
            SqlDataAdapter da = new SqlDataAdapter(sql, cx.c);
            DataSet ds = new DataSet();
        ...
        private void txtRetornoNF_KeyPress(object sender, KeyPressEventArgs e)
        {
            ...
            {
                consultar.comando = "";
                consultar.comando = "select Notafiscal as Quantidade from Chamados where OS = '" + txtRetornoNF.Text + "'";
        ...
        private void btnConcluir_Click(object sender, EventArgs e)
        {
            ...
            {
                int contNFSaida = 0;
                //consultar.comando = "select count(*) as Quantidade from NotaFiscalSaida where NotaSaida = '" + txtNFSaida.Text + "'";
                //consultar.consultarHistorico();
                //contNFSaida = consultar.cont;              

                if (contNFSaida > 0)
                {
                    MessageBox.Show("NOTA FISCAL DE SAÍDA JÁ UTILIZADA.");
                }
                else
                {
                    consultar.DataAtual();
                    ...
                    if (chbPorFora.Checked)
                    {
                        ...
                        consultar.comando = "update NotaPorFora set NotaFiscalSaida = '" + txtNFSaida.Text + "', Status = 'CONCLUIDO' where idNotaPorFora in (" + text + ") ";
                    ...
                    else
                    {
                        ...
                        consultar.comando = "update chamados set NotaFiscalSaida = '" + txtNFSaida.Text + "' where idChamados in (" + text + ") ";
                        //     MessageBox.Show(consultar.comando);
                        consultar.Atualizar();
                        //==================================
                    }
                    ...
                    for (int i = 0; i < dgvParaLançarNF.RowCount; i++)
                    {
                        ...
                        string EAN = dgvParaLançarNF.Rows[i].Cells["EAN"].Value.ToString();
                        ...
                        inserirNotaFiscal(txtNFSaida.Text, NotaFiscal, consultar.dataNormal, Classificacao, CodVarejo, Descricao, QNT, Valor_uni, VALOR_TOTAL, NCM, txtVarejista.Text, EAN, Tipo, out feed);
                    }
                    if (feed == "")
                    {
        ...
        public void inserirNotaFiscal(string NFSaida, string NFEntrada, string Data, string Classificacao, string CodVarejo, string Descricao, string QNT, string Valor_uni, string VALOR_TOTAL, string NCM, string Varejista, string EAN, string Tipo, out string Feed)
        {
            string sql = "";
            try
            {
                sql += " Insert into NotaFiscalSaida (NotaSaida, NotaEntrada, Data, Classificacao, CodVarejo, Descricao, QNT, Valor_uni, VALOR_TOTAL, NCM, Varejista, EAN, Tipo)";
                sql += " Values ( ";
                sql += " '" + NFSaida + "', ";
                sql += " '" + NFEntrada + "', ";
                sql += " '" + Data + "', ";
