...
namespace CRMagazine
{
    ...
    {
        public frmEstoquePainelPedidos()
        {
            InitializeComponent();
        }
        ...
        {
            string sql = "";
            sql += " Select idPedidos, Usuario, OS, NS, Codigo, Descricao, Quantidade as 'Qntd', Status, HoraPedido as 'Hora do Pedido', Situacao from Pedidos where Status like '%AGUARDANDO%' order by HoraPedido";
            cx.Conectar();
        ...
        {
            string sql = "";
            sql += " Select Codigo, Quantidade, Posicao from Estoque where Codigo = '" + Codigo + "' and Posicao != 'PP' order by Quantidade desc";
            cx.Conectar();
            SqlDataAdapter da = new SqlDataAdapter(sql, cx.c);
            DataSet ds = new DataSet();
            da.Fill(ds, "Estoque");
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (Status == "PP-AGUARDANDO")
            {
                if (MessageBox.Show("DESEJA CONCLUIR O PP?", "PERGUNTA", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    consulta.comando = "";
                    consulta.comando = "update Pedidos set Status = 'ATENDIMENTO', HoraAtendimento = '" + consulta.dataCompleta + "' where idPedidos = '" + ID + "'";
                ...
                if (MessageBox.Show("DESEJA CONCLUIR?", "PERGUNTA", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    ...
                    if (valor == "sim")
                    {
                        ...
                        if (total >= 0)
                        {
                            ...
                            consulta.comando = "";
                            consulta.comando = "update Estoque set Quantidade = '" + total + "' where Codigo = '" + Codigo + "' and Posicao = '" + txtRetirar.Text + "'";
                            consulta.Atualizar();
        ...
        public void AtualizaDestino(string qntNaPosicao)
        {
            ...
            consulta.comando = "";
            consulta.comando = "update Estoque set Quantidade = '" + total + "' where Posicao = 'REC' and Codigo = '" + Codigo + "'";
         //   MessageBox.Show(consulta.comando);
            consulta.Atualizar();
        ...
        private void btnCancelar_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("DESEJA CANCELAR O CÓDIGO " + Codigo + "?", "PERGUNTA", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                consulta.comando = "";
                consulta.comando = "update Pedidos set Status = 'CANCELADO', HoraAtendimento = '" + consulta.dataCompleta + "' where idPedidos = '" + ID + "'";
                consulta.Atualizar();
                if (consulta.Retorno == "ok")
                {
                    consulta.LimparControles(this);
        ...
        {
            consulta.comando = "";
            consulta.comando = "Delete from Estoque where idCod in ( " + ID + " )";
