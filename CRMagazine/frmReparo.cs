...
namespace CRMagazine
{
    ...
    {
        public frmReparo(string Usuario, string OS)
        {
            InitializeComponent();
        ...
        {
            consulta.comando = "";
            consulta.comando += "Select COUNT(OS) as Quantidade from Pedidos where (Usuario = '" + lblUsuario.Text + "' or OS = '" + txtOS.Text + "') and Status in ('PP-AGUARDANDO', 'AGUARDANDO', 'ATENDIDO', 'ATENDIMENTO') ";
        ...
        public void ContadordePP()
        {
            ...
            consulta.comando += "AND E.Chamado = C.OS and C.Status in ('PP', 'REPARO', 'AGUARDOBACKUP') and C.Responsavel = '" + lblUsuario.Text + "' ";
            consulta.consultarSimNao();
        ...
        {
            consulta.comando = "";
            consulta.comando += "select count(*) as Quantidade from Chamados where Status in ('PP', 'AGUARDOBACKUP') and Responsavel = '" + lblUsuario.Text + "' ";
            consulta.consultarSimNao();
        ...
        public void ContadordePecaParaDevolver()
        {
            ...
            {
                consulta.comando = "";
                consulta.comando += "Select COUNT(*) as Quantidade from Pedidos where OS = '" + txtOS.Text + "' and Status in ('RECEBIDO') ";
                consulta.consultarSimNao();
       ...
       */
        ...
        private void btnSolicitarPeca_Click(object sender, EventArgs e)
        {
            ...
            {
                frmEstoqueSolicitarPecasInterno c = new frmEstoqueSolicitarPecasInterno(lblUsuario.Text, txtOS.Text, txtSKU.Text, txtNS.Text, "BACKUP");
                c.ShowDialog();
                AtualizaContadores();
                /*
                consulta.Coluna = "OS";
        ...
        public void Concluir()
        {
            ...
            if (lstReparo.Items.Count > 0)
            {
                ...
                stringMontada = "Insert into Tecnica (Tecnico, OS, NS, SKU, Descricao, Defeito, CodPeca, DescPeca, DataReparo) Values ";
                int rows = lstReparo.Items.Count;
                foreach (string item in lstReparo.Items)
                {
                    if (rows > 1)
        ...
        public void InserirRMA()
        {
            try
            {
                string sql = "select Codigo, Descricao from Pedidos where Status = 'RECEBIDO' and OS = '" + txtOS.Text + "' ";
                cx.Conectar();
                SqlCommand cd = new SqlCommand();
                cd.Connection = cx.c;
                cd.CommandText = sql;
                ...
                while (dr.Read())
                {
                    ...
                    consulta.comando += " OS = '" + txtOS.Text + "' and Codigo = '" + Codigo + "' and Status = 'RECEBIDO'";
                    consulta.Atualizar();
        ...
        public void ConferirPosicionamentoPeca(out string feedBack)
        {
            feedBack = "ok";
            consulta.comando = "";
            consulta.comando += "Select COUNT(OS) as Quantidade from Pedidos where Status in ('PP-AGUARDANDO', 'AGUARDANDO', 'ATENDIDO', 'ATENDIMENTO') and OS = '" + txtOS.Text + "'";
            consulta.consultarSimNao();
            if (Convert.ToInt32(consulta.qntNaPosicao) > 0)
            {
        ...
        */
        ...
        private void btnPP_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    frmReparoSolicitarPecasPP c = new frmReparoSolicitarPecasPP(lblUsuario.Text, txtOS.Text, txtSKU.Text, txtNS.Text, "PP");
                    c.ShowDialog();

                    consulta.comando = "";
                    consulta.comando = "select count(*) as Quantidade from AguardoBackup where Chamado = '" + txtOS.Text + "' and Status = 'PENDENTE'";
                    consulta.consultarSimNao();
                    if (Convert.ToInt32(consulta.qntNaPosicao) > 0)
                    {
                        //======Insere na tabela Historico==========================
                        ...
                        consulta.comando = "update Chamados set Status = 'PP', Responsavel = '" + lblUsuario.Text + "' where OS = '" + txtOS.Text + "' AND STATUS = 'REPARO'";
                        consulta.Atualizar();
                        if (consulta.Retorno == "ok")
                        {
        ...
        {
            frmReparoPPRecebido c = new frmReparoPPRecebido(lblUsuario.Text);
            c.ShowDialog();
            AtualizaContadores();
        }
        ...
        {
            frmReparoDevolverPeca c = new frmReparoDevolverPeca(txtOS.Text);
            c.ShowDialog();
            AtualizaContadores();
        }
        ...
        {
            frmReparoConsultarMilkRun c = new frmReparoConsultarMilkRun(lblUsuario.Text, txtOS.Text);
            c.ShowDialog();
            AtualizaContadores();
        }
        ...
        {
            frmReparoChamadoemPP c = new frmReparoChamadoemPP(lblUsuario.Text);
            c.ShowDialog();
            AtualizaContadores();
        }
