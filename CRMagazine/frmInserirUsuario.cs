            ...
            else
            {
                if (rbtUsuarioExistente.Checked == true)
                {
                    if (MessageBox.Show("DESEJA ALTERAR AS PERMISSÕES DO USUÁRIO " + cbxUsuario.Text + " ? ", "Pergunta", MessageBoxButtons.YesNo) == DialogResult.Yes)
                    {
                        ...
                        consulta.comando += " Where Usuario = '" + cbxUsuario.Text + "'";
                        consulta.Atualizar();
                ...
                else if (rbtNovoUsuario.Checked == true)
                {
                    ...
                    {
                        consulta.comando = "insert into Usuarios (Usuario, Senha, Entrada, Vistoria, Reparo, Runin, Expedicao, Consultas, Embalagem, Estoque, Assist, Ajuste, ADM ) values ";
                ...
                else if (rbtDeleteUsuario.Checked == true)
                {
                    ...
                    {
                        consulta.comando = "delete from Usuarios Where Usuario = '" + cbxUsuario.Text + "'";
                        consulta.Atualizar();
                        if (consulta.Retorno == "ok")
                        {
                            LimparControles();
