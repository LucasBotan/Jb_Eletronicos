...
namespace CRMagazine
{
    ...
    {
        public frmEstoqueRecebeProducao()
        {
            InitializeComponent();
        }
        ...
        public void PreencherComboboxStatus()
        {
            ...
            string sql = "";
            sql = "";
            sql += " SELECT DISTINCT Usuario FROM Producao WHERE Status = 'AGUARDANDO'";
            cx.Conectar();
        ...
        public void ListarTudo()
        {
            ...
            sql += " Select idProducao, EAN, Descricao, SKU, Usuario, Data, Hora, Status from Producao where Status = 'AGUARDANDO' AND USUARIO = '" + cboUsuario.Text + "' ";
            cx.Conectar();
            ...
            try
            {
                ...
                if (dgvConsulta.Columns[e.ColumnIndex].Name == "CONCLUIR")
                {
                    if (MessageBox.Show("DESEJA CONCLUIR? ", "Pergunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        ...
                        consulta.DataAtual();
                        consulta.comando = "";
                        consulta.comando = "update Producao set Status = 'RECEBIDO', HoraRecebimento = '" + consulta.dataCompleta + "' where idProducao = '" + ID + "'";
                        consulta.Atualizar();
                        if (consulta.Retorno == "ok")
                        {
                ...
                else if (dgvConsulta.Columns[e.ColumnIndex].Name == "CANCELA")
                {
                    if (MessageBox.Show("DESEJA CANCELAR? ", "Pergunta", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    {
                        consulta.DataAtual();
                        consulta.comando = "";
                        consulta.comando = "update Producao set Status = 'CANCELADO', HoraRecebimento = '" + consulta.dataCompleta + "' where idProducao = '" + ID + "'";
                        consulta.Atualizar();
                        if (consulta.Retorno == "ok")
                        {
                            MessageBox.Show("CANCELAMENTO CONCLUIDO");
