...
namespace CRMagazine
{
    ...
    {
        public frmReparoChamadoemPP(string texto)
        {
            InitializeComponent();
        ...
        public void ListarChamadoAguardo()
        {
            ...
            sql += "from Chamados where Status = 'PP' AND Responsavel = '" + lblUsuario.Text + "' ";
            cx.Conectar();
        ...
        {
            string sql = "";
            sql += "select * from AguardoBackup where Chamado = '" + Chamado + "' and status = 'PENDENTE' ";
            cx.Conectar();
            SqlDataAdapter da = new SqlDataAdapter(sql, cx.c);
            DataSet ds = new DataSet();
            da.Fill(ds, "Chamados");
