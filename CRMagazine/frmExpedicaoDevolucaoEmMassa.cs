...
namespace CRMagazine
{
    ...
    {
        public frmExpedicaoDevolucaoEmMassa()
        {
            InitializeComponent();
        }
        ...
        public void Contar()
        {
            ...
            else
            {
                ...
                consulta.comando = "select sum(convert(numeric,QntRestante)) as Quantidade from NotaFiscal where CodVarejo = '" + txtCodVarejo.Text + "' and QntRestante > 0 and Varejista = '" + cboVarejista.Text + "' ";
                ...
                else
                {
                    try
                    {
                        ...
                        sql += " where CodVarejo = '" + txtCodVarejo.Text + "' and QntRestante > 0 and Varejista = '" + cboVarejista.Text + "' order by CONVERT(date, Data, 103) ASC";
                        cx.Conectar();
        ...
        public void InsereNoBancoNotaPorFora()
        {
            for(int i = 0; i < dgvExpedicao.RowCount; i++)
            {
                ...
                consulta.comando = "";
                consulta.comando += "INSERT INTO NOTAPORFORA (Notafiscal, Classificacao, CodVarejo, Descricao, Qnt, Valor_uni, NCM, Varejista, Status, id_NotaEntrada, Qnt_Restante) VALUES ";
                consulta.comando += " ('" + NotaFiscal + "','" + Classificacao + "','" + CodVarejo + "','" + Descricao + "','" + QNT + "','" + Valor_uni + "','" + NCM + "' ,'" + Varejista + "','PENDENTE','" + id_NotaEntrada + "', '" + QNT_Restante + "') ";
                consulta.Atualizar();
                ...
                if (consulta.Retorno == "ok")
                {
                    ...
                    consulta.comando = "";
                    consulta.comando = "UPDATE NOTAFISCAL SET QNTRESTANTE = '"+ QNT_Restante + "' WHERE IDNOTAFISCAL = '" + id_NotaEntrada + "' ";
                    consulta.Atualizar();
