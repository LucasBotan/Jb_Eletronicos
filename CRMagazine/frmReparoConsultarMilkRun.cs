...
namespace CRMagazine
{
    ...
    {
        public frmReparoConsultarMilkRun(string texto, string NS)
        {
            InitializeComponent();
        ...
        public void ListarPedidoDePeca()
        {
            ...
            string sql = "";
            sql += " Select idPedidos, OS, Codigo, Descricao, Status, HoraPedido as 'Hora do Pedido' from Pedidos where " + ParteTexto + " and Status in ('PP-AGUARDANDO', 'AGUARDANDO', 'ATENDIMENTO', 'ATENDIDO') order by HoraPedido";
            cx.Conectar();
            ...
            try
            {
                if (dgvAguardo.Columns[e.ColumnIndex].Name == "Retira")
                {
                    ...
                    if (Status == "ATENDIDO")
                    {
                        ...
                        {
                            consulta.comando = "";
                            consulta.comando = "update Pedidos set Status = 'RECEBIDO', HoraRecebimento = '" + data + "' where idPedidos = '" + idPedidos + "'";
                            consulta.Atualizar();
                            ListarPedidoDePeca();
